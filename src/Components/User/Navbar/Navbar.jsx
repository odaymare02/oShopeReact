...
export default function Navbar() {
  ...
  return (
    <nav
      className={`navbar navbar-expand-lg fixed-top pt-3 shadow container mt-2 rounded-2 ${
        isScrolled ? "bg-light" : style.isScrolled
      } `}
    >
      <div className="container-fluid">
        <Link
          className={`navbar-brand  text-center m-auto ${style.brand}`}
          href="#"
        >
          O_Shope
        </Link>
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          {isLogin ? (
            <ul className="navbar-nav m-auto mb-2 mb-lg-0">
              ...
              <li className="nav-item">
                <Link className={`nav-link ${style.navLink}`} to={"/AllProd"}>
                  products
                </Link>
              </li>
            </ul>
            ...
            <div className="nav-item ms-auto">
              <div className="auth d-flex">
                <Link
                  className={`${style.no_underline} text-dark btn btn-outline-white `}
                  to={"/login"}
                >
                  {" "}
                  Signin{" "}
                ...
                <Link
                  to={"/register"}
                  className={`${style.no_underline}  btn btn-outline-dark `}
                >
                  Siginup
                </Link>
              </div>
                ...
                <div class="dropdown">
                  ...
                  <ul className="dropdown-menu p-2">
                    <div className=" d-flex gap-2  align-items-center justify-content-start my-2">
                      ...
                      <Link className={`${style.no_underline}`} to={"/profile"}>
                        Profile
                      </Link>
                    </div>
                ...
                <Offcanvas show={show} onHide={handleClose} placement="end">
                  <Offcanvas.Header closeButton>
                    <Offcanvas.Title className="d-flex justify-content-between  w-100">
                      <p>Your Cart</p>
                      <span className="text-secondary">total:{totalPrice}</span>
                    </Offcanvas.Title>
                  </Offcanvas.Header>
                  <Offcanvas.Body>
                    {totalQuantity == 0 ? (
                      ...
                      <div className="row">
                        {cartItems?.map((item) => (
                          <div
                            key={item.id}
                            className="col-12 mb-3 border-bottom position-relative shadow p-2 rounded-3"
                          >
                            {item.details.discount > 0 ? (
                              <p className="position-absolute top-0 end-0 translate-middle badge rounded-pill bg-danger">
                            ...
                            <div className="d-flex justify-content-center align-items-center gap-2">
                              ...
                              <p className="text-secondary">
                                {item.details.name.substring(0, 30)}
                              </p>
